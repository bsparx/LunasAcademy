...
import type { ActivityLevel, Badge } from "@/app/learn/_data/progress-content";
...
export function ProgressClient({ name, summary, activity, badges }: Props) {
  const earned = badges.filter((b) => b.earned);
  const locked = badges.filter((b) => !b.earned);
  const cellMap = cellsToMap(activity);
  const count = contributionCount(activity.cells);
    ...
    <div className="flex min-h-screen bg-[var(--cream-50)]">
      ...
      <div className="flex-1 min-w-0">
        <div className="mx-auto max-w-6xl px-10 py-10 space-y-10">
          ...
          <header className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-[14px] text-[var(--color-ink-500)]">
                Streaks, badges named after minerals, and a full pathway when you finish.
              </p>
            </div>
            ...
            <div className="mt-3 rounded-xl border border-[var(--color-ink-200)]/40 bg-white p-5 shadow-[0_1px_2px_rgba(15,40,30,0.04)]">
              <div className="flex flex-wrap items-baseline justify-between gap-3">
                <div className="text-[12px] text-[var(--color-ink-500)]">
                  ...
                  contribution{count === 1 ? "" : "s"} in the last {activity.weeks} weeks
                  ...
                  {Array.from({ length: activity.daysPerWeek }).map((_, day) => (
                    <div key={day} className="flex gap-[3px]">
                      {Array.from({ length: activity.weeks }).map((__, week) => {
                        const level = cellMap.get(`${week}-${day}`) ?? 0;
                        const daysAgo =
          ...
          {/* BADGES */}
          <section>
            <SectionHeading icon={<Award className="h-4 w-4 text-[var(--color-mint-600)]" />} title="Badges" hint="Named after minerals and gems" />
...
function Cell({
  ...
  const activityLabel = describeLevel(level);
...
function tierSurface(tier: Badge["tier"]): string {
  switch (tier) {
    case "Common":
...
function tierPill(tier: Badge["tier"]): string {
  switch (tier) {
    case "Common":
...
function cellsToMap(grid: Props["activity"]): Map<string, ActivityLevel> {
  const m = new Map<string, ActivityLevel>();
  for (const c of grid.cells) {
    m.set(`${c.week}-${c.day}`, c.level);
  }
  return m;
}

function contributionCount(cells: { level: ActivityLevel }[]): number {
  let n = 0;
  for (const c of cells) {
    if (c.level === "today") n += 1;
    else if (c.level >= 1) n += 1;
  }
  return n;
}

function describeLevel(level: ActivityLevel): string {
  if (level === "today") return "Today";
  if (level === 0) return "No activity";
  return `${level} contribution${level === 1 ? "" : "s"}`;
}
